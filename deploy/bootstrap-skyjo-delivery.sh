...
adopt_legacy() {
  ...
  if [ -e "$target" ] || [ -L "$target" ]; then
    ...
    for item in dist server-dist scripts package.json package-lock.json server.mjs server-account-store.mjs server-game-state-validation.mjs server-invite-codes.mjs server-room-invites.mjs server-room-persistence.mjs server-persistence-health.mjs server-push.mjs server-readiness.mjs server-release.mjs server-state-backup.mjs server-unicode.mjs; do
