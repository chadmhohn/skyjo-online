...
The controller reads the smoke credentials locally only for the post-activation production HTTP and WebSocket check. They are not copied into GitHub and are never accepted as SSH arguments. The canary uses copied state plus random synthetic credentials, forces `127.0.0.1:4181`, has no production environment access, disables push credentials so verification cannot notify real users, and supplies the fixed synthetic Apple application identifier only inside the isolated canary environment.
