...
5. Merge only after that manual canary passes, then require the protected `main` run's normal `CI / Release Canary` to pass again. Do not tag or promote during the cutover window. If bootstrap or the manual canary fails, leave production untouched and repair or execute the prior verified generation's full preparation rollback described below before retrying. The persistent lock lives under the deploy home rather than the stage tree, so creating it is harmless to the old controller's directories-only stage validation.
...
The first deployment of this stable grammar is a bootstrap-before-merge transition because the previously installed controller still uses the historical per-helper list. Freeze and independently review the exact PR-head SHA, snapshot its complete `deploy/` bundle and the existing pinned public keys into a new root-owned immutable bootstrap generation, then require controller self-test plus a signed verify-only localhost canary with `activation=false`. Confirm production identity and readiness remain unchanged before merging. After merge, protected-main certification must exercise the same controller contract again. If any transition check fails, execute the prior generation rollback below; do not activate an application release, restart production, or restore live data as part of this controller-only rollback.

### APNs controller-generation refresh

The first APNs backend release changes privileged `deploy/` controls that are intentionally excluded from the application runtime artifact: explicit blank APNs canary settings, production provider/encryption-key validation, and the canary unit's inaccessible-path contract. Before merging that release, freeze and independently review its exact PR-head SHA, record the current bootstrap generation plus local and public production identity, and prepare a new root-owned immutable bootstrap generation from that SHA's complete `deploy/` bundle and the existing pinned public keys. Require the installed delivery manifest to verify, `/usr/local/sbin/skyjo-release-controller self-test` to report `activation=false`, and a signed verify-only localhost canary against the exact candidate to complete with `activation=false`. Production identity and readiness must remain unchanged; this controller-only refresh must not activate a release, restart production, or restore live data.

Merge only after those checks pass, then require protected `main` to exercise the same controller contract and exact merge SHA again before APNs is enabled. Retain the prior immutable bootstrap generation until that proof completes. If preparation, self-test, canary, or identity comparison fails, execute that prior generation's preparation rollback below and leave APNs disabled.

#### Executable bootstrap-generation rollback

Changing `/usr/local/lib/skyjo-online/bootstrap/current` alone is not a rollback: that symlink does not reinstall the live controller, dispatcher, launchers, units, sudoers, key snapshots, or delivery manifest. Resolve the exact previously verified immutable generation and re-run that generation's own preparation entrypoint with its own snapshotted inputs. The preparation path validates the generation, then acquires the existing host and admission locks before reinstalling and republishing the prior delivery assets:

```sh
prior=/usr/local/lib/skyjo-online/bootstrap/<prior-64-character-generation>
sudo "$prior/bootstrap-skyjo-delivery.sh" prepare \
  "$prior/inputs/transport.pub" \
  "$prior/inputs/canary.pem" \
  "$prior/inputs/production.pem"
```

After preparation succeeds, require `/usr/local/share/skyjo-online/delivery-assets.sha256` to verify every installed live asset hash, byte-compare the installed controller/dispatcher libraries and launchers with the selected prior generation, and require `/usr/local/sbin/skyjo-release-controller self-test` to return `activation=false`. Read back `bootstrap/current` and require its generation name to equal the selected prior generation. Finally, compare the local and public production readiness/version identity captured before the rollback with the post-rollback results; the active application release SHA and readiness must remain unchanged. A failure in any verification is an incomplete controller rollback and must remain fail-closed.
...
Release directories are root-owned and read-only to `skyjo`. Live state is owned by `skyjo` and is never placed below a release. Backups are root-only. Upload staging is mode `1731` on ext4 and uses its directory link count as a fail-closed 32-run admission counter. Each mode-`0700` run directory has a same-owner mode-`0400` `.quota-admitted` file containing the exact run ID; the stage root remains directories-only for backward compatibility. The persistent admission file is a unique empty `root:skyjo-deploy` mode-`0640` regular file beneath the exact root-owned mode-`0755` deploy home. Dispatcher and controller open it read-only with `O_NOFOLLOW`, validate handle/path inode identity, and have `/usr/bin/flock` lock the inherited descriptor. A forced upload acquires that one descriptor before run reservation and retains it across input receipt, publication or idempotent return, failure cleanup, and release; every dispatcher unlink and `rmdir` requires a fresh held-lock proof. Input has a 15-minute total deadline. The descriptor makes serialization cross-process and crash-releasing without a per-run pathname lock, PID file, or stale-owner cleanup. The controller follows host-lock-then-admission-lock ordering and accepts only paths resolved beneath the expected admitted run directory.
...
The focused `npm run smoke:apns-rollback` gate uses a no-argument source wrapper to load the freshly written full release identity, then starts two concurrent copies of the envelope server against a copied APNs-extended schema-2 database and proves exact `/readyz` and `/version` identity plus validated clean shutdown and byte preservation. The release-only runtime artifact carries the same proof as the controller-compatible root helper `server-apns-rollback-proof.mjs`; normal `server.mjs` startup never imports it. `CI / Runtime Artifact` first proves the exact archive, checksum, and SBOM selected for upload are byte-identical to two deterministic rebuilds, then extracts that upload subject and invokes the helper's strict direct CLI with its required expected SHA. Synthetic backup/restore repeats exact-present preservation. Before #204, the ordinary deployment canary must preserve a table-absent copied state; the #204 candidate creates the frozen table only in its copied canary state, and later canaries accept only the exact-present form.

After the separately approved successor promotion, resolve `previous` to the recorded envelope SHA and require its root-owned mode-`0644` deployment metadata bytes (ordered SHA, artifact digest, tag, and one final LF) to equal the exact tag-run upload subject. Run the artifact-carried proof helper directly from that resolved immutable tree as `skyjo-canary` under `env -i`; it uses only private synthetic temporary state and dynamic loopback ports, suppresses raw server diagnostics, and neither reads production state/secrets nor stops or restarts production. This literal installed-`previous` proof, its bounded SHA-bound success result, both promotions, and both public identities must be recorded before #203 closes. Once any live database contains the table, the recorded envelope tag is a permanent code-rollback floor: never select an older `previous`, manually relink an older release, or restore a pre-table database to facilitate code rollback. Merge and a green main canary do not establish this production floor.

APNs provider keys remain outside every artifact and copied-state canary. Install `/etc/skyjo-online/apns-provider.p8` and `/etc/skyjo-online/apns-token.key` as `root:skyjo` mode `0640` beneath root-owned, non-writable `/etc/skyjo-online`; keep the provider key, persistent encryption key, and their values out of command output and evidence. `/etc/skyjo-online.env` enables the provider only when all four `SKYJO_APNS_*` settings are present and point to those exact paths. Partial or malformed production configuration must prevent a successful startup or activation. The canary controller explicitly blanks all four settings and its systemd unit makes `/etc/skyjo-online` inaccessible, so verification cannot read production key material or contact Apple. Promotion proof records only enabled/disabled state and redacted provider outcomes.
