...
`server-apns.mjs` is an allowlisted runtime module because production delivery imports it. The archive never contains the APNs `.p8`, token-encryption key, provider JWT, device token/fingerprint, or an environment file. Artifact integration starts with APNs coherently disabled; the deployment canary additionally blanks all APNs settings and cannot read the production key directory.
