...
IOS-7 adds the stable aggregate `iOS / UI & Accessibility` job. `./scripts/ios-build-test.sh --ui-accessibility` requires no backend or credentials, selects a standard iPhone, large iPhone, and iPad on the newest installed simulator runtime, builds the committed test plan once without signing, and runs the full native-solo/room matrix locally. Automatic CI is path-aware: unrelated changes run no macOS UI job, while native UI changes run a five-test standard-phone smoke selected with `SKYJO_IOS_UI_ACCESSIBILITY_MODE=smoke`. A manual CI dispatch with `ios_ui_mode=full` splits the complete matrix across the exact standard-phone, large-phone, iPad-portrait, and iPad-landscape roles for pre-TestFlight or intentionally broad layout certification. The stable aggregate fails unless its selected scope succeeds.
