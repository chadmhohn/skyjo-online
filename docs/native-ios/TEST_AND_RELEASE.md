...
The hosted iPad-portrait role cold-boots the selected simulator and reapplies and verifies the owned accessibility settings before each of its five pinned tests. It retains every one-test log, summary, test inventory, proof, and `.xcresult`, runs all five children even after a failure, merges them back into the role's canonical `ipad-portrait.xcresult`, and fails unless that merged bundle proves the exact five XCTest identifiers on one destination with zero failures, skips, or expected failures. Other roles and the complete local matrix retain their existing grouped invocations.
