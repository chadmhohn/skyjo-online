...
IOS-10 keeps the recurring gate proportional: normal PR CI builds the three performance tests but skips their repeated samples. Before a TestFlight candidate, enable `SKYJO_RUN_RELEASE_PERFORMANCE=1` in the selected simulator's `launchctl` environment and run only `SkyjoAppUITests/testIOS10ColdLaunchPerformance`, `SkyjoAppUITests/testIOS10EightPlayerRenderPerformance`, and `SkyjoAppUITests/testIOS10ReconnectPerformance` on the standard-phone simulator. Use a Release-optimized test build with code coverage disabled, `ENABLE_TESTABILITY=YES`, and an explicit test-build `SWIFT_ACTIVE_COMPILATION_CONDITIONS="$(inherited) DEBUG"`; this exposes deterministic fixtures without putting them in the normal archive. Cold launch and table presentation each record three XCTMetric samples, and reconnect records five in-process samples. Its deterministic transport retires the live socket, exercises production backoff, creates a replacement socket, re-admits the saved seat, replays the pending action, and waits for authoritative recovery. Treat cold launch above 3 seconds, full app-launch-to-eight-player-table presentation above 5 seconds, the complete reconnect UI round trip above 4 seconds, or rising memory across the reconnect samples as a release defect; compare absolute CPU/memory values only on the same device/runtime. The table has no blocking opening animation, so its animation budget is satisfied by construction, including Reduce Motion. Existing deterministic realtime and room-model tests remain the faster task/socket regression gate.

The 2026-08-21 Release-optimized fixture baseline on stable Xcode 26.6, iOS 26.5, and an iPhone 17 Pro Max simulator passed all three metric tests. Responsive cold launch averaged 0.974 seconds (0.967, 0.971, and 0.983); full launch-to-eight-player-table averaged 4.438 seconds (4.466, 4.427, and 4.420); replacement-socket reconnect and pending-action replay averaged 1.514 seconds (1.525, 1.514, 1.503, 1.520, and 1.506). Reconnect peak physical memory remained within 35.736-35.834 MB, a 98.3 KB spread across the five samples. These measurements exercise optimized production sources through deterministic test-only transport, but they are not the signed TestFlight archive. Pair them with the physical TestFlight session and a ten-minute Allocations/Leaks check; keep sanitized `.xcresult` and `.trace` evidence local rather than making them recurring CI artifacts.
