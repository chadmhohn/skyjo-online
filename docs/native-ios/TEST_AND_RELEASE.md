...
The hosted iPad-portrait role cold-boots the selected simulator and reapplies and verifies the owned accessibility settings before each of its nine pinned tests. The blocked-stats recovery acceptance path is divided into separate blocked-state audit, fresh-fixture retry, corrupt-state audit, corrupt-head discard, and Accessibility XXXL growth tests. The blocked and corrupt audit methods terminate their apps immediately after their final audit, before any scrolling or action can request another UI snapshot; every logical fixture group begins in a fresh XCTest process while retaining its exact assertions and audits. The role retains every one-test log, summary, test inventory, proof, and `.xcresult`, runs all nine children even after a failure, merges them back into the role's canonical `ipad-portrait.xcresult`, and fails unless that merged bundle proves the exact nine XCTest identifiers on one destination with zero failures, skips, or expected failures. Other roles and the complete local matrix retain their grouped invocations.
