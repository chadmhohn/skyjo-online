...
Authenticated account deletion uses exact `DELETE /api/account` JSON with `currentPassword` and uppercase `confirmation: "DELETE"`. The server verifies the current credential, durably scrubs active rooms, transactionally revokes sessions and push registrations, deletes private/solo data, and anonymizes shared multiplayer history before returning `{ "ok": true }` and expiring the account cookie. Native and web clients never invent deletion semantics. See [`ACCOUNT_DELETION.md`](ACCOUNT_DELETION.md) for retention, failure, and backup behavior.
