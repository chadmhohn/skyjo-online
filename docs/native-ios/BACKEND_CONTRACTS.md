...
- Development, tests, and the isolated deployment canary use only `TESTSKYJ01.com.groundworkrevops.skyjo`. The canary is identified by its already-isolated `SKYJO_CANARY_RELEASE_DIR`; that exception does not make the synthetic identifier valid for the production service.
