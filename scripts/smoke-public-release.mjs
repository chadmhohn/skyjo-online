...
async function runOnce(baseUrl, expectedReleaseSha, { allowPreNativeInviteRollback = false } = {}) {
  ...
  if (allowPreNativeInviteRollback) {
    const loginResponse = await fetchPublic(baseUrl, '/login');
    assertPublicResponse(loginResponse, 'pre-native-invite rollback login', /^text\/html\b/i);
    assertNoStore(loginResponse, 'pre-native-invite rollback login');
    assert.match(
      await loginResponse.text(),
      /<form\b[^>]*\baction=["']\/login["']/i,
      'pre-native-invite rollback login form is missing'
    );
  } else {
