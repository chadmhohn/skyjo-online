...
describe('v0.3.5 certification evidence', () => {
...
describe('v0.3.5 immutable tag identity', () => {
  const metadata = {
    tagRef: 'refs/tags/v0.3.5',
    tagName: 'v0.3.5',
    packageVersion: '0.3.5',
    packageLockVersion: '0.3.5',
    packageLockRootVersion: '0.3.5',
    certificationVersion: '0.3.5'
  };
  ...
  function tagIdentityGit(tagContents: string, checkoutCommit = commit) {
    const responses = new Map([
      ['cat-file -t refs/tags/v0.3.5', 'tag\n'],
      ['rev-parse --verify refs/tags/v0.3.5^{tag}', `${tagObject}\n`],
  ...
  it('binds the full tag ref, package, lockfile, and certification versions exactly', () => {
    expect(validateReleaseTagMetadata(metadata)).toBe('v0.3.5');
    for (const changed of [
      { tagRef: 'refs/tags/v0.3.2', tagName: 'v0.3.2' },
      { tagRef: 'refs/tags/v0.3.5-extra' },
      { tagName: 'v0.3.2' },
  ...
  it('accepts only an annotated tag object directly naming the checked-out commit', async () => {
    ...
    expect(runGit).toHaveBeenCalledWith(['rev-parse', '--verify', 'refs/tags/v0.3.5^{tag}']);
    expect(runGit).toHaveBeenCalledWith(['cat-file', 'tag', tagObject]);
    expect(runGit).not.toHaveBeenCalledWith([
      'rev-parse',
      '--verify',
      'refs/tags/v0.3.5^{commit}'
    ]);
  });
  ...
  it('rejects mismatched names, malformed headers, and duplicate reserved headers', async () => {
    const cases = [
      ...
      {
        contents: annotatedTagContents().replace('tag v0.3.5\n', 'type commit\ntag v0.3.5\n'),
...
describe('v0.3.5 workflow governance', () => {
  ...
  it('requires the exact load gate and preserves pinned, least-privilege workflow execution', async () => {
    const [
      ...
      fs.readFile(path.join(root, 'docs', 'releases', 'v0.3.5-certification.md'), 'utf8'),
      fs.readFile(path.join(root, 'docs', 'releases', 'v0.3.5-security.md'), 'utf8'),
    ...
    expect(CERTIFICATION_RELEASE_VERSION).toBe('0.3.5');
    expect(CERTIFICATION_RELEASE_DATE).toBe('2026-08-19');
    ...
    expect(JSON.parse(packageLock).version).toBe('0.3.5');
    expect(JSON.parse(packageLock).packages[''].version).toBe('0.3.5');
    expect(changelog).toMatch(/^## 0\.3\.5 - 2026-08-19$/m);
    expect(changelog).toContain('authenticated native-notification configuration');
    expect(changelog).toContain('encrypted APNs token persistence');
    expect(changelog).toContain('permanent v0.3.3 rollback-floor enforcement');
    ...
    expect(certificationAddendum).toContain('explicit approval in the current conversation naming both exact annotated tag `v0.3.5` and exact `CERT_SHA`');
    expect(certificationAddendum).toContain('v0.3.4 is healthy production `current`');
    expect(certificationAddendum).toContain('v0.3.3 is the permanent minimum code-rollback target');
    expect(certificationAddendum).toContain('close #203');
    expect(certificationAddendum).toContain('#204 is unblocked');
    ...
    expect(certificationAddendum).toContain('--tag v0.3.5');
    ...
    expect(certificationAddendum).toContain('Real permission handling, locked/background iPhone delivery');
    expect(certificationAddendum).toContain('exact immutable v0.3.4');
    ...
    expect(securityAddendum).toContain('AES-256-GCM ciphertext');
    expect(securityAddendum).toContain('fixed sandbox or production origin');
    expect(securityAddendum).toContain('Web Push and APNs are independently settled');
    ...
    expect(securityAddendum).toContain('canary services blank all settings');
    expect(securityAddendum).toContain('permanent v0.3.3 envelope floor');
