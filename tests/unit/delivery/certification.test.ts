...
describe('v0.3.4 certification evidence', () => {
...
describe('v0.3.4 immutable tag identity', () => {
  const metadata = {
    tagRef: 'refs/tags/v0.3.4',
    tagName: 'v0.3.4',
    packageVersion: '0.3.4',
    packageLockVersion: '0.3.4',
    packageLockRootVersion: '0.3.4',
    certificationVersion: '0.3.4'
  };
  ...
  function tagIdentityGit(tagContents: string, checkoutCommit = commit) {
    const responses = new Map([
      ['cat-file -t refs/tags/v0.3.4', 'tag\n'],
      ['rev-parse --verify refs/tags/v0.3.4^{tag}', `${tagObject}\n`],
  ...
  it('binds the full tag ref, package, lockfile, and certification versions exactly', () => {
    expect(validateReleaseTagMetadata(metadata)).toBe('v0.3.4');
    for (const changed of [
      { tagRef: 'refs/tags/v0.3.2', tagName: 'v0.3.2' },
      { tagRef: 'refs/tags/v0.3.4-extra' },
      { tagName: 'v0.3.2' },
  ...
  it('accepts only an annotated tag object directly naming the checked-out commit', async () => {
    ...
    expect(runGit).toHaveBeenCalledWith(['rev-parse', '--verify', 'refs/tags/v0.3.4^{tag}']);
    expect(runGit).toHaveBeenCalledWith(['cat-file', 'tag', tagObject]);
    expect(runGit).not.toHaveBeenCalledWith([
      'rev-parse',
      '--verify',
      'refs/tags/v0.3.4^{commit}'
    ]);
  });
  ...
  it('rejects mismatched names, malformed headers, and duplicate reserved headers', async () => {
    const cases = [
      ...
      {
        contents: annotatedTagContents().replace('tag v0.3.4\n', 'type commit\ntag v0.3.4\n'),
...
describe('v0.3.4 workflow governance', () => {
  ...
  it('requires the exact load gate and preserves pinned, least-privilege workflow execution', async () => {
    const [
      ...
      fs.readFile(path.join(root, 'docs', 'releases', 'v0.3.4-certification.md'), 'utf8'),
      fs.readFile(path.join(root, 'docs', 'releases', 'v0.3.4-security.md'), 'utf8'),
    ...
    expect(CERTIFICATION_RELEASE_VERSION).toBe('0.3.4');
    expect(CERTIFICATION_RELEASE_DATE).toBe('2026-08-18');
    ...
    expect(JSON.parse(packageLock).version).toBe('0.3.4');
    expect(JSON.parse(packageLock).packages[''].version).toBe('0.3.4');
    ...
    expect(certificationAddendum).toContain('explicit approval in the current conversation naming both exact tag `v0.3.4` and exact `CERT_SHA`');
    expect(certificationAddendum).toContain('`previous` resolves to the exact immutable `v0.3.3` tag');
    expect(certificationAddendum).toContain('close #203 and unblock #204');
    ...
    expect(certificationAddendum).toContain('--tag v0.3.4');
