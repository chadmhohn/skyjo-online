...
- Froze and validated the optional APNs device-storage rollback envelope without creating or using its table. Added a controller-compatible, artifact-only synthetic rollback-proof helper with strict exact-SHA CLI binding, sanitized mismatch failures, leak scanning across unique row metadata and binary renderings, and bounded signal/timeout cleanup; normal server startup never imports it. This candidate is only the first release in the required two-promotion rollback-anchor sequence; merge or tag preparation alone does not complete that production gate.
- Refreshed the release audit toolchain to Lighthouse 13.4.1 and patched transitive dependencies, removing every high/critical audit finding. Updated React Router to 6.30.6, rejected unsafe account return paths before navigation, and retained only the two exact reviewed moderate advisories through 2026-09-30.
