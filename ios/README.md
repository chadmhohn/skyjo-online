...
- `TestPlans/SkyjoCI.xctestplan` enables unit, package-graph, resource, launch, no-web-view, UI, and accessibility checks with coverage. Canonical `contracts/v1` HTTP/realtime fixtures, real local-server two-cookie relaunch simulation, separate-session invite-cookie handoff, state-model races, native access/account/stats flows, durable room reset/seat recovery, real Swift/PWA mixed-room scenarios, authenticated native-room navigation, and the IOS-7 multi-device solo UI matrix are committed gates.
