...
- `TestPlans/SkyjoCI.xctestplan` enables unit, package-graph, resource, launch, no-web-view, UI, and accessibility checks with coverage. Canonical `contracts/v1` HTTP/realtime fixtures, real local-server two-cookie relaunch simulation, separate-session invite-cookie handoff, state-model races, native access/account/stats flows, durable room reset/seat recovery, real Swift/PWA mixed-room scenarios, consumed-review invite navigation from Stats and Account, destructive room confirmations, compact opponent geometry, and the IOS-7 multi-device solo UI matrix are committed gates.
...
The project declares `applinks:skyjo.groundworkrevops.com` in its Associated Domains entitlement. That committed source plus simulator coverage is not physical-device proof: the final Apple application identifier, team signing, immutable backend promotion, Apple CDN propagation, and installed/uninstalled iPhone behavior remain explicit IOS-8 release gates.
