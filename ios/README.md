...
The script verifies the Release API origin, builds a clean unsigned device archive for resource inspection, and builds an ad-hoc Xcode-signed Release simulator app whose executable entitlements are audited in every architecture. It installs locked Node dependencies when needed, builds `server-dist`, and launches the real `server.mjs` on a dynamic loopback port without an access password, using generated test-only session/invite secrets and temporary SQLite/room files. It passes only bounded nonsecret loopback/test-mode values to the simulator, performs one unsigned `xcodebuild test`, proves open access, direct account signup, account-cookie recreation, legacy route compatibility, and account/stats requests against that server, terminates the exact child, and deletes the validated temporary state/raw log. Its exit finalizer scans raw evidence for generated secrets and stages only verified sanitized files into the exact ignored directory CI may upload under `ios/Artifacts/`. Credentials and CI tokens never enter the `xcodebuild` environment.
...
UI/accessibility mode needs no backend, account, or credential. It discovers a standard iPhone, large iPhone, and iPad on the newest installed simulator runtime, builds once without signing, then exercises solo launch/setup/table/scoring/settings/recovery plus authoritative waiting/active/scoring/pending/offline/resynchronized room fixtures across phone portrait and short landscape, large phone, and iPad portrait/landscape. The command above runs the complete matrix locally. Ordinary native-UI pull requests and protected-main pushes set `SKYJO_IOS_UI_ACCESSIBILITY_MODE=smoke` with the standard-phone role, which runs five representative launch, solo, summary, account-routing, and multiplayer tests. Workflow-only edits use the cheap placeholders; manually dispatch `smoke` or `full` before merging an edit that actually rewires a native job. A manual CI dispatch with `ios_ui_mode=full` runs all four roles before TestFlight or when broad layout evidence is actually needed. The stable aggregate required check passes only when the selected scope passes, while unrelated changes skip the macOS role entirely. CI uploads compact sanitized text/JSON evidence only on failure for three days; full `.xcresult` bundles remain local instead of becoming routine artifacts.
...
- `Packages/SkyjoNetworking` and `Packages/SkyjoPersistence` depend only on `SkyjoDomain`. `SkyjoAPIClient` composes the actor-owned compatibility `AccessSessionClient` with typed account, profile/password, stats, solo-stats submission, readiness, and version requests on the same injected dedicated `URLSession`/persistent cookie store. It rejects redirects and unexpected final URLs, bounds payloads, requires compatible operational versions, decodes stable `{ code, error }` failures, and uses a safe fallback for unknown or malformed errors. `RoomInviteClient` validates the exact production HTTPS route, keeps signed tokens opaque, manually attaches existing same-origin cookies to a cookie-disabled transport, and commits the single same-host legacy redemption cookie only after a direct, bounded JSON/no-store success has passed DTO and invite-semantic validation. Invite creation never accepts response cookies. Redirects, transport failures, error statuses, invalid headers, oversized bodies, malformed payloads, and off-domain cookies cannot mutate the shared jar. `SkyjoAPIClient` also creates authenticated actor-owned `RoomConnection` instances sharing one account-fenced reset-recovery store. The realtime codec validates exact protocol-v2 frames and hidden-state semantics, and the state machine permits one exact-replay command until acknowledgement/snapshot convergence without optimistic mutation.
- `SkyjoApp/App/AppModel.swift` owns the native account/home/stats navigation state and injects the account-partitioned solo feature. Bootstrap verifies backend compatibility and then loads the optional account directly; the shared-password view has no route. `AppDependencies` also owns the shared-cookie invite client and `RoomAppCoordinator`; room navigation exists only for a currently confirmed online account, while sanitized invite review survives account transitions and retiring room hosts cannot receive a newer link. Request identities and account-generation guards prevent stale async responses from restoring state after logout or account replacement. Password fields are cleared after use, cookies are never surfaced to views, native admin remains a web link, and permanent account deletion requires current-password verification plus exact destructive confirmation before clearing the deleted account's local persistence partition after server success.
...
- `TestPlans/SkyjoCI.xctestplan` enables unit, package-graph, resource, launch, no-web-view, UI, and accessibility checks with coverage. Canonical `contracts/v1` HTTP/realtime fixtures, real local-server open-access/account-cookie relaunch simulation, transactional invite-cookie handoff through the app coordinator, stale-room recovery, state-model races, native account/stats flows, durable room reset/seat recovery, real Swift/PWA mixed-room scenarios, consumed-review invite navigation from Stats and Account, destructive room confirmations, compact opponent geometry, and the multi-device solo-and-room UI matrix are committed gates.
