...
struct RoomSessionModelTests {
  ...
  @Test("Account retirement preserves the latest invite queued during a profile transition")
  func queuedInviteSurvivesProfileTransitionAndAccountRetirement() async throws {
    let accountA = testAccount()
    let updatedAccountA = testAccount(displayName: "Updated Host")
    let accountB = testAccount(
      id: UUID(uuidString: "30000000-0000-4000-8000-000000000004")!,
    ...
    await coordinator.synchronize(account: accountA)
    let transitioningHost = try #require(coordinator.sessionHost)
    await transitioningHost.model.start()
    await accountAConnection.suspendNextDispose()
    let profileTransition = Task {
      await coordinator.synchronize(account: updatedAccountA)
    }
    #expect(await modelEventually { await accountAConnection.disposeIsSuspended() })

    let firstURL = URL(
      string: "https://skyjo.groundworkrevops.com/invite/first_profile.first_signature"
    )!
    let latestURL = URL(
      string: "https://skyjo.groundworkrevops.com/invite/latest_profile.latest_signature"
    )!
    let firstInvite = try RedeemedRoomInvite(
      roomCode: "ABCDE",
      expiresAt: 1_784_999_000_000
    ...
    await redemption.resume(request: 1, with: firstInvite)
    #expect(await firstAcceptance.value)
    let latestAcceptance = Task { await coordinator.accept(latestURL) }
    #expect(await modelEventually { await redemption.count() == 2 })
    await redemption.resume(request: 2, with: latestInvite)
    ...
    await accountAConnection.resumeDispose()
    await profileTransition.value
    #expect(coordinator.sessionHost === replacementHost)
  ...
  @Test("A live invite quarantines an automatically recovered seat before its first snapshot")
  func liveInviteQuarantinesAutomaticRecoveryBeforeFirstSnapshot() async throws {
    let connection = ModelRoomConnection()
    ...
    let seededSaveCount = await seatStore.saveCount()
    let model = makeModel(connection: connection, seatStore: seatStore)
    let recoveredRoom = try await authoritativeFixture(revision: 7, variant: .waiting)

    await model.start()
    #expect(await connection.admissions() == [
      .join(code: "ABCDE", displayName: "Host", playerID: "seat-old"),
    ])
    ...
    await connection.emit(.snapshot(recoveredRoom))

    #expect(!(await modelEventually(attempts: 30) { model.room != nil }))
    #expect(model.pendingInviteReview == invite)
    #expect(model.joinCode == "FGHIJ")
    #expect(await seatStore.saveCount() == seededSaveCount)
    ...
    #expect(await seatStore.saveCount() == seededSaveCount + 1)
    await model.stop()
  }

  @Test("A live invite quarantines a manually recovered seat before its first snapshot")
  func liveInviteQuarantinesManualRecoveryBeforeFirstSnapshot() async throws {
    let connection = ModelRoomConnection()
    let seatStore = RecordingSeatStore()
    let account = testAccount()
    let model = makeModel(connection: connection, seatStore: seatStore)
  ...
  func staleAdmissionNoticesAreAttemptScoped() async throws {
    ...
    for noticeKind in [NoticeKind.interrupted, .rejected] {
      ...
      let oldJoin = Task { await model.join(code: "ABCDE") }
      #expect(await modelEventually { await connection.connectIsSuspended() })
      let oldAttemptIDs = await connection.admissionAttemptIDs()
      let oldAttemptID = try #require(oldAttemptIDs.first)

      model.applyInvite(invite)
      await connection.resumeConnect()
      #expect(!(await oldJoin.value))

      await connection.suspendNextConnect()
      let switchTask = Task { await model.acceptInviteAndJoin() }
      #expect(await modelEventually { await connection.connectIsSuspended() })
      let attemptIDs = await connection.admissionAttemptIDs()
      #expect(attemptIDs.count == 2)
      let targetAttemptID = try #require(attemptIDs.last)
      #expect(targetAttemptID != oldAttemptID)

      let staleNotice: RoomConnectionNotice = switch noticeKind {
      case .interrupted:
        .freshAdmissionInterrupted(admissionAttemptID: oldAttemptID)
      case .rejected:
        .admissionRejected(
          code: "room-not-found",
          message: "Retired admission detail.",
          usedSavedSeat: false,
      ...
      await connection.emit(.status(connectedStatus(revision: targetRoom.revision)))
  ...
  @Test("Canceling an invite restores the authoritative admission it superseded")
  func cancelInviteRestoresSupersededAdmission() async throws {
    let connection = ModelRoomConnection()
    let seatStore = RecordingSeatStore()
    let model = makeModel(connection: connection, seatStore: seatStore)
    ...
    #expect(await connection.actions() == [.startGame])
    await model.stop()
  }

  @Test("Canceling an invite falls back to the connection's authoritative snapshot")
  func cancelInviteUsesConnectionSnapshotFallback() async throws {
    let connection = ModelRoomConnection()
    let seatStore = RecordingSeatStore()
    let model = makeModel(connection: connection, seatStore: seatStore)
    let admittedRoom = try await authoritativeFixture(revision: 7, variant: .waiting)
    ...
    model.applyInvite(invite)
    await connection.installAuthoritativeSnapshotWithoutEmitting(admittedRoom)

    await model.dismissInviteReview()

    #expect(await connection.authoritativeSnapshotReadCount() == 1)
  ...
  @Test("Canceling an invite without a snapshot disconnects and preserves cleanup failure")
  func cancelInviteWithoutSnapshotHandlesDisconnectFailure() async throws {
    let connection = ModelRoomConnection()
    ...
    #expect(!model.canSubmitAdmission)
  ...
  @Test("Saved-seat actions distinguish confirmed absence, recovery failure, and cleanup")
  func savedSeatAndCleanupActionVisibilityIsDeterministic() async throws {
    let noSeatConnection = ModelRoomConnection()
    let noSeatModel = makeModel(connection: noSeatConnection)

    await noSeatModel.start()
    #expect(await noSeatConnection.persistedResetAttemptIDs().count == 1)
    #expect(noSeatModel.savedSeatKnownAbsent)
    #expect(!noSeatModel.shouldShowRetrySavedSeat)
    #expect(!noSeatModel.shouldShowForgetSavedSeat)
    #expect(!noSeatModel.canRetrySavedSeat)
    #expect(!noSeatModel.canForgetSavedSeat)
    await noSeatModel.stop()

    let seededConnection = ModelRoomConnection()
    await seededConnection.failRecovery(.connectionUnavailable)
    let seededStore = RecordingSeatStore()
    let account = testAccount()
    try await seededStore.save(RoomSeatRecoveryRecord(
      accountID: account.id,
      roomCode: "ABCDE",
      playerID: "seat-old"
    ))
    let seededModel = makeModel(connection: seededConnection, seatStore: seededStore)

    await seededModel.start()
    #expect(seededModel.banner?.title == "Saved room unavailable")
    #expect(!seededModel.savedSeatKnownAbsent)
    #expect(seededModel.shouldShowRetrySavedSeat)
    #expect(seededModel.shouldShowForgetSavedSeat)
    #expect(seededModel.canRetrySavedSeat)
    #expect(seededModel.canForgetSavedSeat)
    await seededModel.stop()

    let rejectedConnection = ModelRoomConnection()
    let rejectedStore = RecordingSeatStore()
    try await rejectedStore.save(RoomSeatRecoveryRecord(
      accountID: account.id,
      roomCode: "ABCDE",
      playerID: "seat-old"
    ))
    let rejectedModel = makeModel(
      ...
      seatStore: rejectedStore
    )

    await rejectedModel.start()
    let rejectedAttemptID = try #require(
      await rejectedConnection.admissionAttemptIDs().last
    )
    await rejectedConnection.emit(.notice(.admissionRejected(
      code: "room-not-found",
      message: "Sensitive server detail.",
      usedSavedSeat: true,
      admissionAttemptID: rejectedAttemptID
    )))
    #expect(await modelEventually {
      rejectedModel.banner?.title == "Saved seat unavailable"
    })
    #expect(await rejectedStore.record() == nil)
    #expect(rejectedModel.savedSeatKnownAbsent)
    #expect(!rejectedModel.shouldShowRetrySavedSeat)
    #expect(!rejectedModel.shouldShowForgetSavedSeat)
    await rejectedModel.stop()

    let cleanupConnection = ModelRoomConnection()
    let cleanupModel = makeModel(connection: cleanupConnection)
    await cleanupModel.start()
    #expect(cleanupModel.savedSeatKnownAbsent)

    await cleanupConnection.emit(.notice(.resetRecoveryPersistenceFailed))
    #expect(await modelEventually {
      cleanupModel.banner?.title == "Room reset cleanup needed"
    })
    #expect(!cleanupModel.shouldShowRetrySavedSeat)
    #expect(cleanupModel.shouldShowForgetSavedSeat)
    #expect(!cleanupModel.canRetrySavedSeat)
    #expect(cleanupModel.canForgetSavedSeat)
    #expect(!cleanupModel.canSubmitAdmission)

    await cleanupModel.forgetSavedSeat()
    #expect(cleanupModel.banner == nil)
    #expect(cleanupModel.savedSeatKnownAbsent)
    #expect(!cleanupModel.shouldShowRetrySavedSeat)
    #expect(!cleanupModel.shouldShowForgetSavedSeat)
    #expect(!cleanupModel.canRetrySavedSeat)
  ...
  func realtimeStatusLatencyIsBounded() async throws {
    let connection = ModelRoomConnection()
    let model = makeModel(connection: connection)
    ...
    await model.start()
    await model.createRoom()
    let admissionAttemptID = try #require(await connection.admissionAttemptIDs().last)
  ...
  @Test("A successful routing clear canceled by a newer invite restores the current saved seat")
  func canceledSuccessfulClearResavesCurrentRoomWithoutBroadcast() async throws {
    let connection = ModelRoomConnection()
    let seatStore = SuspendedSeatClearStore()
    let model = makeModel(connection: connection, seatStore: seatStore)
    let currentRoom = try await authoritativeFixture(revision: 7, variant: .playing)
    let inviteB = try RedeemedRoomInvite(
      roomCode: "FGHIJ",
      expiresAt: 1_784_999_100_000
    )
    ...
    await model.start()
    await connection.emit(.status(connectedStatus(revision: 7)))
    await connection.emit(.snapshot(currentRoom))
    #expect(await modelEventually { await seatStore.record()?.roomCode == "ABCDE" })
    ...
    #expect(model.snapshot?.revision == 7)
    #expect(model.pendingInviteReview == inviteC)
    #expect(await modelEventually { await seatStore.record()?.roomCode == "ABCDE" })
    #expect(await connection.admissions().isEmpty)
    await model.stop()
  }
  ...
  @Test("A live reset-recovery failure invalidates an admission before its first snapshot")
  func liveResetRecoveryFailureInvalidatesInFlightAdmission() async throws {
    let connection = ModelRoomConnection()
    let seatStore = RecordingSeatStore()
    let model = makeModel(connection: connection, seatStore: seatStore)
    let lateRoom = try await authoritativeFixture(revision: 7, variant: .waiting)

    await model.start()
    await connection.suspendNextConnect()
    let joinTask = Task { await model.join(code: "ABCDE") }
    #expect(await modelEventually { await connection.connectIsSuspended() })

    await connection.emit(.notice(.resetRecoveryPersistenceFailed))
    #expect(await modelEventually { model.banner?.title == "Room reset cleanup needed" })
    #expect(!model.canSubmitAdmission)
    #expect(model.canForgetSavedSeat)

    await connection.resumeConnect()
    #expect(!(await joinTask.value))
    await connection.emit(.status(connectedStatus(revision: 7)))
    await connection.emit(.snapshot(lateRoom))
    #expect(!(await modelEventually(attempts: 30) { model.room != nil }))
    #expect(await seatStore.record() == nil)
    await model.stop()
  }

  @Test("A reset-recovery notice consumed during Forget cannot invalidate verified cleanup")
  func resetRecoveryFailureDuringForgetPreservesCleanupOwnership() async {
    let connection = ModelRoomConnection()
    await connection.failDisconnect(.seatCleanupUnavailable)
    await connection.suspendNextDiscard()
    let model = makeModel(connection: connection)

    await model.start()
    let forgetTask = Task { await model.forgetSavedSeat() }
    #expect(await modelEventually { await connection.discardIsSuspended() })
    ...
    #expect(await modelEventually { model.connectionStatus.phase == .connecting })
    #expect(model.isAdmissionOperationPending)
    #expect(model.banner == nil)

    await connection.resumeDiscard()
    await forgetTask.value
    #expect(!model.isAdmissionOperationPending)
    #expect(model.banner == nil)
    await connection.emit(.status(idleStatus()))
    #expect(await modelEventually { model.connectionStatus.phase == .idle })
    #expect(model.canSubmitAdmission)
    await model.stop()
  }

  @Test("Verified reset cleanup ignores its delayed failure notice and re-arms for a new reset")
  func verifiedResetCleanupFencesDelayedFailureNotice() async throws {
    let connection = ModelRoomConnection()
    await connection.failDisconnectWithDelayedResetRecoveryNotice()
    let model = makeModel(connection: connection)
    let waiting = try await authoritativeFixture(revision: 7, variant: .waiting)
    ...
    await connection.releaseDelayedEvents()
    await connection.emit(.status(RoomConnectionStatus(
      phase: .connecting,
      retryInMilliseconds: nil,
      synchronized: false,
    ...
    await connection.emit(.snapshot(waiting))
    #expect(await modelEventually { model.commandsEnabled })
    await model.resetRoom()
    #expect(await connection.actions() == [.resetRoom])
    await connection.emit(.notice(.resetRecoveryPersistenceFailed))
    #expect(await modelEventually { model.banner?.title == "Room reset cleanup needed" })
    #expect(!model.commandsEnabled)
    await model.stop()
  ...
  @Test("Unresolved reset recovery gates every room mutation until explicit Forget succeeds")
  func unresolvedResetRecoveryGatesRoomMutations() async throws {
    let connection = ModelRoomConnection()
    await connection.failPersistedResetRecovery(.seatCleanupUnavailable)
    let seatStore = RecordingSeatStore()
    let inviteFactory = RecordingInviteFactory()
    let account = testAccount()
    let model = RoomSessionModel(
      account: account,
      environment: RoomSessionEnvironment(
        makeConnection: { connection },
        createInvite: { roomCode in try await inviteFactory.create(roomCode: roomCode) },
        seatStore: seatStore,
        nowMilliseconds: { 1_784_998_800_000 }
      )
    )
    ...
    #expect(!model.commandsEnabled)
    #expect(!model.canCreateShareInvite)
    #expect(!model.canAcceptInvite)
    await model.startGame()
    await model.createShareInvite()
    #expect(await connection.actions().isEmpty)
    #expect(await inviteFactory.requestedRoomCodes().isEmpty)
  ...
  @Test("Forget cannot claim reset cleanup after its connection has retired")
  func forgetWithoutConnectionKeepsResetRecoveryBlocked() async {
    let connection = ModelRoomConnection()
    await connection.failPersistedResetRecovery(.seatCleanupUnavailable)
    let model = makeModel(connection: connection)
  ...
  @Test("An A to B to A switch drains an older suspended routing clear")
  func accountSwitchABADrainsRetiredAdmissionCleanup() async throws {
    let accountA = testAccount()
    let accountB = testAccount(
      id: UUID(uuidString: "30000000-0000-4000-8000-000000000004")!,
      displayName: "Guest"
    ...
    let bConnection = ModelRoomConnection()
    let seatStore = SuspendedSeatClearStore()
    let oldSeat = try RoomSeatRecoveryRecord(
      accountID: accountA.id,
      roomCode: "ABCDE",
      playerID: "seat-old"
    )
    await seatStore.save(oldSeat)
    let factory = HostRoomModelFactory(
      connectionsByAccount: [
        accountA.id: [firstAConnection, secondAConnection],
        accountB.id: [bConnection],
      ],
      sharedSeatStore: seatStore
    )
    let host = RoomSessionHost(account: accountA) { factory.makeModel(for: $0) }
    let retiringModel = host.model

    await retiringModel.start()
    #expect(await firstAConnection.admissions() == [
      .join(code: oldSeat.roomCode, displayName: accountA.displayName, playerID: oldSeat.playerID),
    ])
    let oldAdmission = Task { await retiringModel.join(code: "FGHIJ") }
    #expect(await modelEventually { await seatStore.clearIsSuspended() })

    let switchToBCompletion = ModelCompletionProbe()
    let switchToB = Task {
      await host.synchronize(account: accountB)
      await switchToBCompletion.markComplete()
    }
    #expect(await modelEventually { await firstAConnection.disposed() })

    let switchToACompletion = ModelCompletionProbe()
    let switchToA = Task {
      await host.synchronize(account: accountA)
      await switchToACompletion.markComplete()
    }
    #expect(!(await modelEventually(attempts: 25) {
      await switchToACompletion.isComplete()
    }))
    #expect(!(await switchToBCompletion.isComplete()))
    #expect(host.model === retiringModel)

    await seatStore.resumeClear()
    #expect(!(await oldAdmission.value))
    await switchToB.value
    await switchToA.value
    #expect(await switchToACompletion.isComplete())

    let freshModel = host.model
    #expect(freshModel !== retiringModel)
    #expect(freshModel.account.id == accountA.id)
    let newerSeat = try RoomSeatRecoveryRecord(
      accountID: accountA.id,
      roomCode: "KLMNO",
    ...
    await seatStore.save(newerSeat)
    await freshModel.start()

    #expect(await secondAConnection.admissions() == [
      .join(
        code: newerSeat.roomCode,
...
private actor RecordingInviteFactory {
  private var roomCodes: [String] = []

  func create(roomCode: String) throws -> NativeRoomInvite {
    roomCodes.append(roomCode)
    return NativeRoomInvite(
      roomCode: roomCode,
      url: URL(string: "https://skyjo.groundworkrevops.com/invite/fixture.signature")!,
      expiresAt: 1_784_999_100_000
    )
  }
...
private actor ModelRoomConnection: RoomSessionConnection {
  ...
  private var receivedAdmissionAttemptIDs: [UUID] = []
  private var persistedResetAdmissionAttemptIDs: [UUID] = []
  ...
  private var recoveryFailure: ModelTestError?
  ...
  private var delaysResetRecoveryFailureOnDisconnect = false
  private var shouldSuspendNextConnect = false
  private var shouldSuspendNextDisconnect = false
  private var shouldSuspendNextDiscard = false
  ...
  private var suspendedDiscardContinuation: CheckedContinuation<Void, Never>?
  ...
  func currentAuthoritativeSnapshot() -> AuthoritativeRoomSnapshot? {
    authoritativeSnapshotReads += 1
    return latestAuthoritativeSnapshot
  }
  func recoverPersistedReset(admissionAttemptID: UUID) throws -> Bool {
    persistedResetAdmissionAttemptIDs.append(admissionAttemptID)
    if let persistedResetFailure { throw persistedResetFailure }
    return false
  }

  func connect(_ admission: RoomAdmission, admissionAttemptID: UUID) async {
    receivedAdmissionAttemptIDs.append(admissionAttemptID)
  ...
  func recover(_ admission: RoomAdmission, admissionAttemptID: UUID) throws {
    receivedAdmissionAttemptIDs.append(admissionAttemptID)
    receivedAdmissions.append(admission)
  ...
  func disconnect() async throws {
    ...
    if delaysResetRecoveryFailureOnDisconnect {
      delaysResetRecoveryFailureOnDisconnect = false
      delayedEvents.append(.notice(.resetRecoveryPersistenceFailed))
    }
  ...
  func discardPersistedResetRecovery() async throws {
    discardCalls += 1
    if shouldSuspendNextDiscard {
      shouldSuspendNextDiscard = false
  ...
  func installAuthoritativeSnapshotWithoutEmitting(_ snapshot: AuthoritativeRoomSnapshot) {
    latestAuthoritativeSnapshot = snapshot
  }

  func releaseDelayedEvents() {
    for event in delayedEvents { continuation.yield(event) }
    delayedEvents = []
  }

  func admissions() -> [RoomAdmission] { receivedAdmissions }
  func admissionAttemptIDs() -> [UUID] { receivedAdmissionAttemptIDs }
  func persistedResetAttemptIDs() -> [UUID] { persistedResetAdmissionAttemptIDs }
  ...
  func failRecovery(_ error: ModelTestError?) { recoveryFailure = error }
  func failDisconnect(_ error: ModelTestError?) { disconnectFailure = error }
  func failDisconnectWithDelayedResetRecoveryNotice() {
    disconnectFailure = .seatCleanupUnavailable
    delaysResetRecoveryFailureOnDisconnect = true
  }
  func failDiscard(_ error: ModelTestError?) { discardFailure = error }
  func authoritativeSnapshotReadCount() -> Int { authoritativeSnapshotReads }
  ...
  func suspendNextDiscard() { shouldSuspendNextDiscard = true }
  func discardIsSuspended() -> Bool { suspendedDiscardContinuation != nil }
  func resumeDiscard() {
    suspendedDiscardContinuation?.resume()
    suspendedDiscardContinuation = nil
  }
...
private final class HostRoomModelFactory: @unchecked Sendable {
  ...
  private let sharedSeatStore: (any RoomSeatRecoveryStore)?

  init(
    connectionsByAccount: [UUID: [ModelRoomConnection]],
    sharedSeatStore: (any RoomSeatRecoveryStore)? = nil
  ) {
    self.connectionsByAccount = connectionsByAccount
    self.sharedSeatStore = sharedSeatStore
  }
  ...
  func makeModel(for account: AccountUser) -> RoomSessionModel {
    ...
    return RoomSessionModel(
      ...
      environment: RoomSessionEnvironment(
        makeConnection: { connection },
        createInvite: { _ in throw ModelTestError.inviteUnavailable },
        seatStore: sharedSeatStore ?? RecordingSeatStore(),
        nowMilliseconds: { 1_784_998_800_000 }
      )
    )
  }
