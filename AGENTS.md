...
- External account-deletion ledger: `/var/lib/skyjo-online/account-deletions.json`, via optional `SKYJO_ACCOUNT_DELETION_LEDGER_FILE`. It is not a rollback/backup payload; startup, canaries, and isolated restores reapply its non-PII tombstones.
...
- APNs persistence uses a two-release rollback rule: the schema-2 envelope release must first be immutably promoted and confirmed as `previous`; the feature release then creates the exact optional `apns_devices` table transactionally without advancing the public migration ledger. Never roll back past the verified envelope after the table exists.
...
- `server-account-deletion-ledger.mjs`: external non-PII deletion tombstones. It prevents verified pre-deletion backups, copied canary state, or startup reconciliation from resurrecting deleted accounts and related private state.
...
- `ios/SkyjoApp/Features/Solo/`: native guest/account-partitioned solo launcher, explicit replacement, 1-7-bot setup, pure-reducer game loop, stable board-first iPhone/iPad table, scoring/settings, audio/haptics, lifecycle autosave, and visible stats-outbox recovery.
...
- `scripts/ios-ui-accessibility-test.sh`: credential-free native-solo XCUITest matrix. It discovers a standard phone, large phone, and iPad on the newest simulator runtime, builds once, exercises portrait/landscape and accessibility configurations, and retains sanitized `.xcresult` evidence and screenshots.
...
- The GitHub repo is public. Never commit `.env`, `/etc/skyjo-online.env`, cookies, passwords, session secrets, APNs provider/encryption keys, device tokens or fingerprints, tunnel tokens, room dumps with private content, or OpenClaw secrets.
...
- `server.mjs` changes.
- `server-account-store.mjs`, `server-apns.mjs`, or account/session/game-history/push API changes.
